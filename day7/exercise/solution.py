"""
Day 7 EXERCISE SOLUTION — programmatic check + escalation.

Filled-in reference for day7/exercise/capstone.py. Only the three
TODO(exercise) blocks differ from the starter.
"""
...
# ─── SOLUTION #1 — deterministic check with three sub-rules. ─────────────────
def programmatic_check(draft: str, question: str) -> tuple[bool, str]:
    ...
    if cites < MIN_CITATIONS:
        return False, f"only {cites} citation(s); need ≥ {MIN_CITATIONS}"
    if len(draft or "") < MIN_DRAFT_CHARS:
        return False, f"too short ({len(draft or '')} < {MIN_DRAFT_CHARS} chars)"
    keyword = max(re.findall(r"[A-Za-z]{4,}", question or ""), key=len, default="")
    if keyword and keyword.lower() not in (draft or "").lower():
        return False, f"off-topic: draft doesn't mention '{keyword}'"
    return True, f"{cites} citations · on topic · {len(draft)} chars"
...
# ─── SOLUTION #2 — require BOTH signals; escalate on low confidence. ─────────
def _route_after_reflect(state) -> str:
    revisions = state.get("revisions", 0)
    scores = state.get("scores", []) or [state.get("score", 0.0)]

    if revisions >= MAX_REVISIONS:
        print("    ⛔  hard cap → escalate to human")
        return "approval"

    if len(scores) >= 2 and (scores[-1] - scores[-2]) < MIN_DELTA:
        print(f"    ⛔  plateau ({scores[-2]:.2f}→{scores[-1]:.2f}) → escalate")
        return "approval"

    verdict_pass = state.get("verdict") == "PASS"
    if verdict_pass and state.get("check_ok") and state.get("score", 0.0) >= LOW_CONFIDENCE:
        return "approval"   # auto-publish candidate — still needs a click

    if verdict_pass and state.get("score", 0.0) < LOW_CONFIDENCE:
        print(f"    ⚠  low confidence ({state['score']:.2f}) → ESCALATE")
        return "approval"

    print(f"    ↻  revise (verdict={state.get('verdict')} check_ok={state.get('check_ok')})")
    return "write"
